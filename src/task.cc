...
body_omp_task_target(
    ...
    // the current task is the implicit task, that was polling instructions progression.
    // little hack here: set the current task as 'task' so libomptarget sees it
    xkrt_thread_t * thread = xkrt_thread_t::get_tls();
    assert(thread);

    assert(thread->current_task != task);
    task_t * prev_task = thread->current_task;
