...
__kmpc_taskgroup(ident_t *loc, int gtid)
{
    ...
    // TODO: implement taskgroup in XKRT
    //  - add a new 'TASK_FLAG_TASKGROUP'
    //  - add a new 'task_grp_info_t' with a pointer to taskgroup of the task
    //  - add a new attribute in thread_t that is the currently active 'taskgroup_t'
    //  - a 'taskgroup_t' should have 1 single attribute: the number of spawned
    //    but incomplete tasks, so that it can be waited on in__kmpc_end_taskgroup
    //  - when a task with 'TASK_FLAG_TASKGROUP' is created, set
    //  'task_grp_info_t' to the taskgroup active in the current thread
    //
    // Before that, we want to reduce the size of 'task_get_extra_size' that is
    // generated by the python script. It currently grows exponentially, while
    // many configurations are not practical and should be culled-out
}
...
__kmpc_end_taskgroup(ident_t *loc, int gtid)
{
    ...
    // TODO: after implementing taskgroup, instead of waiting for all tasks,
    // just wait for the taskgroup counter
    xkomp_t * xkomp = xkomp_get();
    xkomp->runtime.task_wait();
